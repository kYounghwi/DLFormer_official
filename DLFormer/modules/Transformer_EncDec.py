...
class Encoder(nn.Module):
    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None):
        ...
        return x, attns # x, [(batch, head, LF, LF) for _ in encoder_layer]
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None, tau=None, delta=None):
        ...
        inp, attns = self.cross_attention(x, cross, cross, attn_mask=cross_mask, tau=tau, delta=delta)
        # inp, [(batch, head, Term, LF) for _ in decoder_layer]
        x = x + self.dropout(inp)
        y = x = self.norm2(x)
